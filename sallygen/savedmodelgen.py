...
def main():
    ...
    step_model = tf.saved_model.load('demo_model')
    #step_model = step(model_reloaded, chars_from_ids, ids_from_chars)

    def run_model(context):
        start = time.time()
        states = None
        next_char = tf.constant([context])
        result = [next_char]
